    ...
    override fun createTransferUnsignedBody(
        ...
        if (messageType == MessageType.Extension) {
            return CellBuilder.createCell {
                ...
                val query = queryId?.toTonBigInt() ?: 0.toBigInt()
                storeUInt(query, 64)
        ...
        val signingMessage = CellBuilder.beginCell().storeUInt(opCode.code, 32)
            .storeSlice(context.cell().beginParse())
        if (seqno == 0) {
            for (i in 0 until 32) {
        ...
        signingMessage.storeSlice(
            W5Action.storeOutListExtendedV5R1(gifts.toList(), messageType).beginParse()
        )
